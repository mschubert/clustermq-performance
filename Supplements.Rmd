...
authenticated using a one-time password passed as environment variable in the
...
`clustermq` and the other packages, it should be noted that both test cases
focussed on a high number of short-running function calls to be evaluated via
HPC schedulers.

There are of course many use cases that do not require that many calls to be
evaluated. In this case, and especially if also the duration of each call
becomes longer, the `batchtools` package will be an excellent choice.

The difference in processing time we see can only in part be
explained by the physical limitations of the network vs. the file system. A
major contributor is that both `BatchJobs` and `batchtools` save the result of
each function call in a separate results file irrespective of chunking, which
causes additional latency that could be avoided by combining the results of a
chunk in a single results file.
...
job's wall time limit), all computed results should still be available.
`clustermq` makes no such guarantees. It is the user's responsibility to
reserve enough wall time, and computations that can not be completed will be
lost.

Hence, both packages have their strengths and weaknesses and should be used in
